...
gb_lines <- readLines("Neottieae_all.gb")
...
dir.create("neot_orchid_split_gb", showWarnings = FALSE)
...
for (i in seq_along(record_starts)) {
  ...
  outfile <- file.path("neot_orchid_split_gb", paste0(taxon, ".gb"))
...
gb_files <- list.files("neot_orchid_split_gb", pattern = "\\.gb$", full.names = TRUE)
